...
\begin{enumerate}
    ...
    \item Take the sequence of prime numbers \(2,3,5,7,11,\dotsc\) and form the number
        \begin{eqna}
            2^{\tilde{S}_0}\cdot3^{\tilde{S}_1}\cdot5^{\tilde{S}_2}\cdot7^{\tilde{S}_3}\cdot11^{\tilde{S}_4}\cdot\dotsb
        \end{eqna}
\end{enumerate}
...
Some sentences, for example \propositio!I N N it_rains it_rains! which means ``if it does not hold that it doesn't rain, it rains,'' seem to be universally valid. It's useful to form a special set of some such  sentences, called axioms, which we will implicitly assume. We'll figure out which sentences are useful to take as axioms later.% except in fuzzy situations (fuzzy logic)
...
\begin{eqnb}
    \text{false}\rightarrow\text{false}=\text{true},\quad\text{false}\rightarrow\text{true}&=&\text{true},\nonumber\\
    \text{true}\rightarrow\text{false}=\text{false},\quad\text{true}\rightarrow\text{true}&=&\text{true}.\nonumber
\end{eqnb}
...
Let
\begin{eqna}
    f:\lbrace x:x\:\text{is a word}\rbrace\rightarrow\lbrace\text{false},\text{true}\rbrace
\end{eqna}
...
\end{proof}
The proof used a technique called mathematical induction: we first showed that \(X_0\) is true. Then we showed that if \(X_n\) is true with \(n\leq m\), then also \(X_{m+1}\). Starting from \(X_0\) this condition step by step ensures that all \(X_n\)'s are true. %must show that if A = B > C, then B and C are unique.
